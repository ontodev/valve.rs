...
sqlite_test: build/valve.db test/src/table.tsv | test/output
	...
	./valve test/src/table.tsv load build/valve.db
	test/round_trip.sh $^
...
pg_api_test: valve test/src/table.tsv test/insert_update.sh | test/output
	...
	# We drop all of the db tables because the schema for the next test (random test) is different
	# from the schema used for this test.
	./$< $(word 2,$^) drop-all $(pg_connect_string)
	@echo "Test succeeded!"
...
sqlite_random_test: valve random_test_data | build test/output
	...
	./$< $(random_test_dir)/table.tsv load $(sqlite_random_db)
...
pg_random_test: valve random_test_data | build test/output
	...
	./$< $(random_test_dir)/table.tsv load $(pg_connect_string)
...
penguin_command_sqlite = ./valve src/schema/table.tsv load --initial-load penguins.db
penguin_command_pg = ./valve src/schema/table.tsv load $(pg_connect_string)
penguin_command_pg_drop = ./valve src/schema/table.tsv drop-all $(pg_connect_string)
...
$(perf_test_db): valve perf_test_data $(perf_test_dir)/*.tsv | build $(perf_test_dir)/ontology
	rm -f $@
	time -p ./$< --verbose --interactive $(perf_test_dir)/table.tsv load --initial-load $@
...
pg_perf_test: valve $(perf_test_dir)/ontology | test/output
	time -p ./$< --verbose --interactive $(perf_test_dir)/table.tsv load $(pg_connect_string)
