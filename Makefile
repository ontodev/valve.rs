...
sqlite_test: build/valve.db test/src/table.tsv | test/output
	...
	./valve load test/src/table.tsv build/valve.db
	test/round_trip.sh $^
...
pg_api_test: valve test/src/table.tsv test/insert_update.sh | test/output
	...
	./$< load $(word 2,$^) $(pg_connect_string)
	./$< test-api $(word 2,$^) $(pg_connect_string)
	...
	# We drop all of the db tables because the schema for the next test (random test) is different
	# from the schema used for this test.
	./$< drop-all $(word 2,$^) $(pg_connect_string)
	@echo "Test succeeded!"
...
sqlite_random_test: valve random_test_data | build test/output
	...
	./$< load $(random_test_dir)/table.tsv $(sqlite_random_db)
...
pg_random_test: valve random_test_data | build test/output
	...
	./$< load $(random_test_dir)/table.tsv $(pg_connect_string)
...
penguin_command_sqlite = ./valve load src/schema/table.tsv --initial-load penguins.db
penguin_command_pg = ./valve load src/schema/table.tsv $(pg_connect_string)
penguin_command_pg_drop = ./valve drop-all src/schema/table.tsv $(pg_connect_string)
