...
use chrono::Utc;
use crossbeam;
use csv::{QuoteStyle, ReaderBuilder, StringRecord, StringRecordsIter, WriterBuilder};
use enquote::unquote;
...
impl Valve {
    ...
    pub fn sort_tables(
        ...
        let constraints_config = self
            ...
            .ok_or(ValveError::ConfigError(
                "Unable to retrieve configured constraints.".into(),
            ))?;
    ...
    pub fn save_tables(
        ...
        for (table, path) in table_paths.iter() {
            let columns: Vec<&str> = self
                ...
                .and_then(|v| Some(v.iter().map(|i| i.as_str().unwrap()).collect()))
                .unwrap();
    ...
    pub fn save_table(
        ...
        // TODO: Do some validation on the path.
