...
pub async fn rename_db_table_tx(
    db_kind: &DbKind,
    config: &ValveConfig,
    tx: &mut Transaction<'_, sqlx::Any>,
    table: &str,
    ...
    // Although PostgreSQL supports ALTER VIEW, SQLite does not, so we have to manually drop
    // and recreate:
    ...
    let sql = get_sql_for_standard_view(new_name, db_kind);
    let query = sqlx_query(&sql);
    query.execute(tx.acquire().await?).await?;
    let sql = get_sql_for_text_view(&config.table, table, Some(new_name), db_kind);
    let query = sqlx_query(&sql);
    query.execute(tx.acquire().await?).await?;
...
/// errors. If `base_table` is given, then the view will be constructed over it, otherwise it will
/// be built over table. In either case, `table` will be used to look up the table's columns. This
/// is useful in case there is a mismatch between the configured table name and the name of the
/// corresponding database table, which can occur during the process of renaming a table.
pub fn get_sql_for_text_view(
    ...
    let base_table = match base_table {
        None => table,
        Some(name) => name,
    };
